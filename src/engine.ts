...
/**
 * A reversal books postings equal and opposite to its target's, at the
 * TARGET's value date. That is all it does.
 *
 * It does not delete or amend the target -- the ledger is append-only, and the
 * target remains a true statement about what was booked. It does not reverse a
 * fee its target caused, recreate interest that was never earned, or release a
 * hold. Each of those would be a separate decision with its own authorization;
 * see AMBIGUITIES section 9, and the tests that pin every one of them.
 */
function decideReversal(ledger: Ledger, event: ReversalEvent): LedgerRecord {
  const target = ledger.records.find((record) => record.event.id === event.reverses);

  // Reaching into another account's history is not a business decline, it is a
  // hole. Fail loudly rather than recording a plausible-looking rejection.
  if (target && target.event.accountId !== event.accountId) {
    throw new Error(
      `reversal ${event.id} names account ${event.accountId}, but its target ` +
        `${event.reverses} belongs to account ${target.event.accountId}`,
    );
  }

  // A target that was rejected posted nothing, so there is nothing to negate.
  if (!target || target.decision !== "APPLIED") {
    return ledger.append({
      event,
      decision: "REJECTED",
      reason: "NO_SUCH_EVENT" satisfies RejectionReason,
      postings: [],
    });
  }

  const alreadyReversed = ledger.records.some(
    (record) =>
      record.decision === "APPLIED" &&
      record.event.kind === "REVERSAL" &&
      record.event.reverses === event.reverses,
  );
  if (alreadyReversed) {
    return ledger.append({
      event,
      decision: "REJECTED",
      reason: "ALREADY_REVERSED" satisfies RejectionReason,
      postings: [],
    });
  }

  // A declared value date that its target does not have is a contradiction in
  // the instruction. Silently preferring one would hide a feed bug behind a
  // posting that looks right.
  if (event.valueDate !== target.event.valueDate) {
    throw new Error(
      `reversal ${event.id} declares value date ${event.valueDate}, but its ` +
        `target ${event.reverses} is valued on day ${target.event.valueDate}`,
    );
  }

  const currency = ledger.currencyOf(event.accountId);
  return ledger.append({
    event,
    decision: "APPLIED",
    postings: target.postings.map((posting) => ({
      ...
      amount: negated(currency, posting.amount),
    })),
  });
}
...
export function ingest(ledger: Ledger, event: LedgerEvent): LedgerRecord {
  switch (event.kind) {
    ...
    case "REVERSAL":
      return decideReversal(ledger, event);
    default:
